...
ElementComponent: Предсатвляет собой класс обёртку для всех элементов в объекте класса Component.
...
set contentElement - устанавливает контент модального окна
...
set error: Устанавливает и отображает ошибки валидации формы.
set isValid: Управляет доступностью кнопки отправки в зависимости от валидности данных.
...
_price - отображает общую стоимость товаров в корзине
_listItems - позволяет отображать список карточек товаров

Методы:
set listItems - устанавливает список товаров в корзине
set price - устанавливает общую сумму продуктов в корзине
set valid - добавляет кнопке атрибут disabled
...
ContactForm — это класс, который наследует функциональность от класса Form и предназначен для работы с формой контактных данных. Он реализует интерфейс IContactsForm.

Атрибуты:
_phoneNumber: Это поле для ввода номера телефона
_email : Это поле для ввода почты
...
_address - Этот элемент представляет собой поле для ввода адреса
...
getProduct: Он получает данные о конкретном товаре и возвращает объект с информацией о товаре.
getProducts: Этот метод получает данные о каталоге товаров и возвращает массив объектов.
...
_catalog: Этот объект содержит весь список продуктов.
...
set catalog: Этот метод используется для инициализации списка товаров, полученных с сервера.

Слой Presenter

Основные события:

/^contacts\..*:change/ - Обновляет контактную информацию в приложении при изменении соответствующих полей.
/^order\..*:change/ - Обновляет адрес заказа в приложении при изменении соответствующего поля.
card:select - Выбирает карточку товара и устанавливает превью товара.
basket:open - Открывает модальное окно с содержимым корзины.
basket:change - Обновляет список товаров в корзине и количество товаров на странице.
contacts:submit - Отправляет контактную информацию.
preview:change - Открывает модальное окно с информацией о товаре.
catalog:change - Изменяет список товаров и вызывает перерисовку каталога на странице.
formErrors:change - Обновляет статус валидации и ошибки для формы оплаты и контактов при изменении ошибок в форме заказа.
order:open - Открывает модальное окно с выбором оплаты и вводом адреса
