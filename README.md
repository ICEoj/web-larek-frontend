...
product:select - Выбирает карточку товара и устанавливает превью товара.
...
order:success - Показывает окно с подтверждением успешной оплаты заказа.
items:change - Изменяет список товаров и вызывает перерисовку каталога на странице.
